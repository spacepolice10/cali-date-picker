...
// import & create Dateform component
  	import { Timeform } from 'spc-calendar'
	 ...
	 // pass params (date & callback to change date)
		...
		date={date}
		onChange={(date: Date) => `updateArgs(date)`}
	>
...
In this package `timeform` consists of 2 main fragments:

- hook `useTimeform` that generates necessary data to implement clocks with picker functionality.
- component `Timeform` that behaves as a minimal representation of `useTimeform` functionality. It returns date-params through render-prop, so developers have full control over styling & behavior of component in general.
